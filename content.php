	...
	<div class="entry-footer">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
			...
			<span class="cat-links">
				<?php printf( __( 'Posted in %1$s', 'creare-boilerplate' ), $categories_list ); ?>
			</span>
			...
			<span class="tags-links">
				<?php printf( __( 'Tagged %1$s', 'creare-boilerplate' ), $tags_list ); ?>
			</span>
			<?php endif; // End if $tags_list ?>
		<?php endif; // End if 'post' == get_post_type() ?>

		<?php if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
		<span class="comments-link"><?php comments_popup_link( __( 'Leave a comment', 'creare-boilerplate' ), __( '1 Comment', 'creare-boilerplate' ), __( '% Comments', 'creare-boilerplate' ) ); ?></span>
		<?php endif; ?>
